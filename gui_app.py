...
class MainWindow(QMainWindow):
    ...
    def toggle_saving(self, checked):
        if checked:
            ...
            if not fname:
                QMessageBox.warning(self, "Warning", "Please select a CSV filename in the Settings tab.")
            ...
            if os.path.exists(fname):
                QMessageBox.information(self, "Appending Data", f"The file '{fname}' already exists.\nData will be appended to it.")
            
            # Clear historical data to synchronize plot with new CSV session
            self.times.clear()
            for i in range(1, 4):
    ...
    def reset_plot_view(self):
        ...
        for ax, _ in self.canvas.ax_gas_list:
    ...
    def _update_data_only(self):
        """Update line data in-place without recreating axes. Preserves zoom/pan state."""
        ...
        for ax, sensors in self.canvas.ax_gas_list:
            max_val = 0.01
            for i in sensors:
                data_list = list(self.historical_data[i]['gas'])
                key = ('gas', i)
                if key in self._lines:
                    self._lines[key].set_data(times_list, data_list)
                valid = [v for v in data_list if not np.isnan(v)]
                if valid:
                    max_val = max(max_val, max(valid))

            # Autoscale X always (to follow latest data), Y only when auto
            ax.relim()
        ...
        if self.canvas.ax_temp:
            for i in range(1, 4):
                key = ('temp', i)
                if key in self._lines:
                    self._lines[key].set_data(times_list, list(self.historical_data[i]['temp']))
            self.canvas.ax_temp.relim()
            self.canvas.ax_temp.autoscale_view(scalex=True, scaley=self.auto_y_scale)
        ...
        if self.canvas.ax_hum:
            for i in range(1, 4):
                key = ('hum', i)
                if key in self._lines:
                    self._lines[key].set_data(times_list, list(self.historical_data[i]['hum']))
            self.canvas.ax_hum.relim()
            self.canvas.ax_hum.autoscale_view(scalex=True, scaley=self.auto_y_scale)
